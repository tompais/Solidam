...
function validarDonaciones() {
    ...
    switch (tipoDonacion) {
        case 1:
            var monto = $("#monto").val();
            var cbu = $("#cbu").val();
            if (monto === null || monto.length === 0 || monto <= 0) {
        ...
        case 2:
            var j = 0;
            while (j < i && valor) {
                if ($("#nombreInsumos-" + j + "").val() === null || $("#nombreInsumos-" + j + "").val() === "" || $("#nombreInsumos-" + j + "").val().length === 0)
                    valor = false;
                else
                    if ($("#cantInsumos-" + j + "").val() === null || $("#cantInsumos-" + j + "").val().length === 0 || $("#cantInsumos-" + j + "").val() <= 0)
                        valor = false;
                j++;
            }
            if (!valor) {
        ...
        case 3:
            ...
            if (cantHoras === null || cantHoras.length === 0 || cantHoras <= 0) {
